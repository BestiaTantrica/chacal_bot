...
def analyze_bot(name, db_path):
    if not os.path.exists(db_path):
        return {"name": name, "total": 0, "profit_abs": 0, "win_rate": 0, "status": "Offline/Missing"}
...
def cmd_status():
    ...
    for name, path in DB_FILES.items():
        ...
        if data["status"] == "Offline/Missing":
    ...
    if idle_bots:
        report += "💤 *Bots en Espera:* " + ", ".join([f"`{b}`" for b in idle_bots]) + "\n"
    if offline_bots:
        report += "💀 *Bots Offline:* " + ", ".join([f"`{b}`" for b in offline_bots]) + "\n"
...
def cmd_report():
    ...
    global_wins = sum(b['wins'] for b in results.values() if 'wins' in b)
...
def cmd_audit():
    ...
    if not found:
        return "📭 No hay datos históricos para auditar en ningún bot."
    return msg
...
def cmd_daily():
    ...
    if detail:
        ...
        msg += "\n_Aún no hay cierres registrados hoy_"
    return msg
...
def cmd_balance():
    ...
    msg += f"Capital Base: `$300.00`\n"
    msg += f"Capital Actual: `${current:.2f}`\n"
    msg += f"Diferencia: `${total_profit:+.2f}` (`{total_profit/300*100:+.1f}%`)\n\n"
    msg += "*Detalle por Bot:*\n" + "\n".join(detail)
    return msg
...
def cmd_winners():
    ...
    if not results: return "🌙 No hay ganadores acumulados todavía."
...
def cmd_losers():
    ...
    if not results: return "✅ Todos los bots están en verde o sin trades."
    
    ranked = sorted(results, key=lambda x: x["profit_abs"])
    msg = "*⚠️ BOTS EN REVISIÓN*\n\n"
    for bot in ranked:
...
def run_comandante():
    ...
    if not token or not chat_id:
        log("Error: No se encontraron credenciales de Telegram.")
        return
    ...
    # Registrar comandos
    commands = [
        {"command": "status", "description": "📍 Estado actual"},
        {"command": "report", "description": "📊 Reporte general"},
        {"command": "audit", "description": "🔍 Auditoría detallada"},
        {"command": "daily", "description": "📅 Hoy"},
        {"command": "balance", "description": "💰 Balance total"},
    ...
    send_telegram("🚀 *Comandante Chacal V3.0 en línea.*\nPanel de control actualizado y optimizado para la Manada.", token, chat_id, keyboard)
